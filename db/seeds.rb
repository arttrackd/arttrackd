...
MaterialPurchase.create!(user_id: 1, name: "Canvas", description: "3 canvases for paintings", cost: 30, units: 3, units_remaining: 3)
...
500.times do
  ...
  MaterialPurchase.create!(user_id: [1,2,3,4,5].sample, name: Faker::Commerce.product_name, description: Faker::Commerce.product_name, cost: Faker::Number.between(0, 50), units: units, units_remaining: units)
end

100.times do
  MaterialUse.create!(material_purchase_id: material_purchases.sample.id, project_id: projects.sample.id, units: Faker::Number.between(0, 1000), user_id: [1,2,3,4,5].sample)
end
